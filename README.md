...
| PRINTF_ONLY_FCTPRINTF                   | NO      | Build the core library, exposing only `fctprintf_` and `vfctprintf_`: These functions don't depend on `putchar_` being available at link-time.|
